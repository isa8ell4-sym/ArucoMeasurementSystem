...
def makeDir(): # only works for CMD
    now = datetime.datetime.now()

    basePath = f"C:/Users/irosenstein/Documents/Vision/Projects/AprilTagDetection/PhotoDetection/processing/{now.month}.{now.day}_{now.hour}.{now.minute}/"
...
def processImages(images, filteredPath, stepperPath, env='CMD'):
    ...
    for fname in images:
        ...
        undImg = undistortCropWidePhotos(fname)
...
def undistortImages(images, stepperPath):
    for img in images:
        ...
        undImg = undistortCropWidePhotos(img)
        cv2.imwrite(os.path.join(stepperPath, f'{name}.JPG'), undImg)


def detectTagsImages(images, detectionPath, env='CMD'):
...
if __name__ == '__main__':
    ...
    basePath, filterPath, stepperPath, detectionPath = makeDir()
    ...
    pd = detectTagsImages(undistortedImages, detectionPath)
    df = DataFrame({'Img Name': pd.names, 'Tag ID': pd.ids, 'x': pd.x, 'y': pd.y, 'z':pd.z, 'theta': pd.theta})
    df.to_excel(f'{basePath}/PositionData.xlsx', sheet_name='sheet1', index=False)
    cv2.waitKey(0)
